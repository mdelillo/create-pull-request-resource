...
func main() {
	...
	remoteRepo := request.Source.RemoteRepository
	forkedRepo := request.Source.ForkedRepository
	location := filepath.Join(os.Args[1], request.Params.RepoLocation)
	accessToken := request.Source.GithubToken
	...
	branchName, prNumber, err := newPullRequest.CreatePullRequest(remoteRepo, forkedRepo, location, accessToken, github.GithubClient{})
	if err != nil {
